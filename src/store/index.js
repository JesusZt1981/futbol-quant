...
function aliasesFor(team) {
  ...
  const stripped = original.replace(/\s+\([A-Z]{2,4}\)\s*$/i,'').replace(/\s+\d+-\d+\s+pen\.?\s*$/i,'').trim();
  const noClubSuffix = stripped.replace(/\s+(FC|CF|SC|AFC)$/i,'').trim();
  return [...new Set([original,stripped,noClubSuffix,...(EXTRA_ALIASES[original]||[]),...(EXTRA_ALIASES[stripped]||[]),...(EXTRA_ALIASES[noClubSuffix]||[])].filter(Boolean))];
}
...
async function pagedMatches({league=null,select='*',limit=10000,cutoff=null}={}) {
  ...
  for(let from=0;from<limit;from+=pageSize){
    ...
    if(league)q=q.eq('league_key',league); if(cutoff)q=q.lt('kickoff',cutoff);
    q=q.order('kickoff',{ascending:false});
    const{data,error}=await q.range(from,Math.min(from+pageSize-1,limit-1));if(error)throw error;
...
async function insert(table,row){
  if(!client){const item={id:crypto.randomUUID(),created_at:new Date().toISOString(),...row};memory[table]=memory[table]||[];memory[table].push(item);return item;}
  const payload={app_key:'futbol_quant',category:table,payload:row,observed_at:row.observed_at||new Date().toISOString()};
  const{data,error}=await client.from('app_history').insert(payload).select().single();if(error)throw error;return data;
}
...
async function list(table,limit=100){
  ...
  if(table)q=q.eq('category',table);const{data,error}=await q;if(error)throw error;
...
async function dataSummary(){
  if(!client)return[];const{data,error}=await client.from('fq_match_summary').select('league_key,competition,matches,finished,from_date,to_date').order('competition',{ascending:true});if(error)throw error;
...
async function teams(league){const rows=await pagedMatches({league,select:'home_team,away_team,kickoff',limit:10000});const set=new Set();for(const r of rows){if(r.home_team)set.add(r.home_team);if(r.away_team)set.add(r.away_team);}return[...set].sort((a,b)=>a.localeCompare(b,'es'));}
function avg(xs){return xs.length?xs.reduce((a,b)=>a+b,0)/xs.length:0;}
function shrink(v,n,b,prior=6){return(v*n+b*prior)/(n+prior);}
function clamp(v,lo,hi){return Math.max(lo,Math.min(hi,v));}
function expectedSeason(league,date=new Date()){const y=date.getUTCFullYear();if(CALENDAR_YEAR_LEAGUES.has(league))return String(y);const m=date.getUTCMonth()+1;return m>=7?`${y}-${String(y+1).slice(-2)}`:`${y-1}-${String(y).slice(-2)}`;}
function perspectiveRates(rows,aliases){const vals=rows.map(r=>{const h=aliases.includes(r.home_team);return{gf:Number(h?r.home_score:r.away_score),ga:Number(h?r.away_score:r.home_score)};});return{n:vals.length,gf:avg(vals.map(x=>x.gf)),ga:avg(vals.map(x=>x.ga))};}
...
function blend(parts){const v=parts.filter(p=>p&&p.weight>0),t=v.reduce((s,p)=>s+p.weight,0)||1;return{attack:v.reduce((s,p)=>s+p.attack*p.weight,0)/t,defense:v.reduce((s,p)=>s+p.defense*p.weight,0)/t};}
...
async function teamRows(aliases,{league=null,season=null,cutoff=null,limit=40}={}){
  ...
  const one=async col=>{let q=client.from('fq_matches').select(select).eq('finished',true).in(col,aliases).not('home_score','is',null).not('away_score','is',null);if(league)q=q.eq('league_key',league);if(season)q=q.eq('season',season);if(cutoff)q=q.lt('kickoff',cutoff);const{data,error}=await q.order('kickoff',{ascending:false}).limit(limit);if(error)throw error;return data||[];};
  const[h,a]=await Promise.all([one('home_team'),one('away_team')]);const map=new Map();for(const r of[...h,...a])map.set(`${r.kickoff}|${r.home_team}|${r.away_team}`,r);return[...map.values()].sort((x,y)=>String(y.kickoff).localeCompare(String(x.kickoff))).slice(0,limit);
}

async function inferDomesticContext(team,competitionLeague,cutoff){
  const aliases=aliasesFor(team),rows=await teamRows(aliases,{cutoff,limit:180});
  const domestic=rows.filter(r=>!CONTINENTAL_COMPETITIONS.has(r.league_key)&&r.league_key!==competitionLeague),counts=new Map();
  for(const r of domestic)counts.set(r.league_key,(counts.get(r.league_key)||0)+1);
...
async function predictionInput(league,home,away,opts={}){
  if(!client)throw new Error('Supabase no está conectado');
  const cutoff=opts.cutoff||new Date().toISOString(),cutoffDate=new Date(cutoff);
  ...
  const clean=rows.filter(r=>r.home_score!=null&&r.away_score!=null);if(clean.length<30)throw new Error('Todavía no hay suficiente historial para esta competición');
  const hAliases=aliasesFor(home),aAliases=aliasesFor(away),baseRows=clean.slice(0,Math.min(clean.length,700));
  ...
  const competition=clean[0]?.competition||league,season=expectedSeason(league,cutoffDate);

  if(!CONTINENTAL_COMPETITIONS.has(league)){
    const hRecent=clean.filter(r=>hAliases.includes(r.home_team)).slice(0,20),aRecent=clean.filter(r=>aAliases.includes(r.away_team)).slice(0,20);if(hRecent.length<3||aRecent.length<3)throw new Error('Muestra reciente insuficiente para uno de los equipos');
    ...
    const hRecentIdx={attack:shrink(hr.gf,hr.n,baseHome,6)/Math.max(baseHome,.35),defense:shrink(hr.ga,hr.n,baseAway,6)/Math.max(baseAway,.35)},aRecentIdx={attack:shrink(ar.gf,ar.n,baseAway,6)/Math.max(baseAway,.35),defense:shrink(ar.ga,ar.n,baseHome,6)/Math.max(baseHome,.35)};
    const hw=hc.n?Math.min(.65,.35+hc.n*.075):0,aw=ac.n?Math.min(.65,.35+ac.n*.075):0,hs=blend([{...indexFrom(hc,baseTeam,4),weight:hw},{...hRecentIdx,weight:1-hw}]),as=blend([{...indexFrom(ac,baseTeam,4),weight:aw},{...aRecentIdx,weight:1-aw}]);
  ...
  const htRows=clean.filter(r=>hAliases.includes(r.home_team)||hAliases.includes(r.away_team)).slice(0,12),atRows=clean.filter(r=>aAliases.includes(r.home_team)||aAliases.includes(r.away_team)).slice(0,12),ht=perspectiveRates(htRows,hAliases),at=perspectiveRates(atRows,aAliases);
  const hSeason=hd.domesticLeague?expectedSeason(hd.domesticLeague,cutoffDate):null,aSeason=ad.domesticLeague?expectedSeason(ad.domesticLeague,cutoffDate):null,hcRows=hd.rows.filter(r=>r.season===hSeason).slice(0,8),acRows=ad.rows.filter(r=>r.season===aSeason).slice(0,8),hUse=hcRows.length?hcRows:hd.rows.slice(0,8),aUse=acRows.length?acRows:ad.rows.slice(0,8),hc=perspectiveRates(hUse,hd.aliases),ac=perspectiveRates(aUse,ad.aliases);
  const hDomBase=hd.rows.length?avg(hd.rows.slice(0,100).flatMap(r=>[Number(r.home_score),Number(r.away_score)])):baseTeam,aDomBase=ad.rows.length?avg(ad.rows.slice(0,100).flatMap(r=>[Number(r.home_score),Number(r.away_score)])):baseTeam;
  ...
  const hTw=ht.n>=3?.32:(ht.n?.15:0),aTw=at.n>=3?.32:(at.n?.15:0),hCw=hc.n?Math.min(.60,.32+hc.n*.055):0,aCw=ac.n?Math.min(.60,.32+ac.n*.055):0;
  const hs=blend([{...hTournament,weight:hTw},{...hCurrent,weight:hCw},{attack:1,defense:1,weight:Math.max(.15,1-hTw-hCw)}]),as=blend([{...aTournament,weight:aTw},{...aCurrent,weight:aCw},{attack:1,defense:1,weight:Math.max(.15,1-aTw-aCw)}]);
...
async function bootstrap(){const s=await dataSummary();return{ok:true,total_rows:s.reduce((a,r)=>a+Number(r.finished||0),0),competitions:s.length,note:'La base histórica está precargada en Supabase.'};}
...
async function settlePredictionAudit(id,homeGoals,awayGoals){if(!client)throw new Error('Supabase no está conectado');const hg=Number(homeGoals),ag=Number(awayGoals),actual=hg>ag?'L':hg===ag?'E':'V';const{data:cur,error:re}=await client.from('fq_prediction_audit').select('predicted_outcome').eq('id',id).single();if(re)throw re;const{data,error}=await client.from('fq_prediction_audit').update({home_goals:hg,away_goals:ag,actual_outcome:actual,correct:cur.predicted_outcome===actual,status:'final',settled_at:new Date().toISOString()}).eq('id',id).select().single();if(error)throw error;return data;}
